...
在机器学习中，密度估计可通过参数估计来直接获得$p(\boldsymbol{x})$；回归任务一般直接估计密度$p(y\mid \boldsymbol{x})$（见后面的参数估计角度的线性回归，属于“判别式模型”）；分类任务可通过参数估计来获得类条件（似然）密度$p(\boldsymbol{x}\mid C)$（见后面的贝叶斯分类器，它是“生成式模型”）。
