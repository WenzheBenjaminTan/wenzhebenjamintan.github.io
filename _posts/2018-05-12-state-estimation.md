...
$$\mathbf{u}_k$$是控制向量，一般情况下我们认为它是关于$$\mathbf{x}_{k-1}$$的函数。
...
粒子滤波（particle filter）是贝叶斯滤波的一种非参数实现，其思想基于模特卡洛仿真，用粒子集来表示概率，可以用在任何形式的状态空间模型上。粒子滤波的前提条件是状态转移函数和观测函数已知，但是过程噪声和观测噪声的分布可以是未知的（均值为0）。
