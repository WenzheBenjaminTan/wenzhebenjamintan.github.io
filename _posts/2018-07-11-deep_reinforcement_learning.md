...
$$y^{DQN} = r + \gamma Q(s',arg\max_a Q(s',a;\boldsymbol{\theta}^-); \boldsymbol{\theta}^-))$$
...
$$y^{DoubleDQN} = r + \gamma Q(s',arg\max_a Q(s',a;\boldsymbol{\theta}); \boldsymbol{\theta}^-))$$
...
$$J(\boldsymbol{\theta}) = E\left[\sum_{t=0}^{T-1}\gamma^tr_{S_t,A_t}(S_{t+1})\mid\pi_{\boldsymbol{\theta}}\right] = \sum_{\tau} R(\tau)P(\tau\mid\boldsymbol{\theta})$$
...
$$\nabla_{\boldsymbol{\theta}} J(\boldsymbol{\theta}) \approx \frac{1}{m}\sum_{i=1}^m \sum_{t=0}^{T-1} \gamma^t R_t^{(i)} \nabla_{\boldsymbol{\theta}}\log\pi_{\boldsymbol{\theta}}(s_t^{(i)},a_t^{(i)})
$$
...
$$\nabla_{\boldsymbol{\theta}} J(\boldsymbol{\theta}) \approx \frac{1}{m}\sum_{i=1}^m \sum_{t=0}^{T-1} (\gamma^t R_t^{(i)} - b)\nabla_{\boldsymbol{\theta}}\log\pi_{\boldsymbol{\theta}}(s_t^{(i)},a_t^{(i)})
$$
...
$$\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} + \frac{\alpha}{m} \sum_{i=1}^m \sum_{t=0}^{T-1} (\gamma^t R_t^{(i)} - b)\nabla_{\boldsymbol{\theta}}\log\pi_{\boldsymbol{\theta}}(s_t^{(i)},a_t^{(i)})
$$
