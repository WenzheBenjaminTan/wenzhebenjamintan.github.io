...
$R$是$A$上的等价关系当且仅当$R$是$A$上的一个自反的、对称和传递的二元关系。$R$是$A$上的序关系当且仅当$R$是$A$上的一个传递的且在$A$上满足三分律的二元关系。
...
形式语言是与自然语言不同，具有精确格式表达要求的语言。使用形式语言可以自然语言的不精确性和模糊性，当然这并不是没有代价的：形式语言的表达能力会受到很大的限制。
...
如果在命题逻辑系统中$$< A_1,A_2,...,A_n >$$是这样一个序列：对于每个$i(1\leq i\leq n)$，$$A_i$$或者是一个公理，或者由序列中前面的两个公式$$A_j, A_k(j < i,k < i)$$应用推理规则而得到，我们就称$$< A_1,A_2,...,A_n >$$是对$$A_n$$的一个证明。并且，称$$A_n$$是该命题逻辑系统内的定理（简称定理或内定理），简记为$$\vdash A_n$$。
...
(g) 命题函数：单独的谓词不是命题，在谓词后面的括号中填上个体或谓词的标识符所得到的式子称为谓词填式。如果在谓词填式的括号中填入的是常元，则该谓词填式就是一个命题常元。如果在谓词填式的括号中填入的是变元，则称该谓词填式为原子命题函数。用逻辑联结符号把一个或多个原子命题函数联结而成的表达式称为复合命题函数。
...
一阶谓词逻辑系统的推理规则与命题逻辑系统一致，只使用modus ponens规则。
